...
class Interfaz(tk.Tk):
    def __init__(self, icono_image_path="./fonts/icono.png"):
        ...
        self.label1 = tk.Label(self, text="Géneros:", font=("Corleone Due", 30), bg="lightblue")
        self.label1.pack(pady=5)
        self.dropdown1 = ttk.Combobox(self, values=["Seleccione opciones"], state="readonly")
        self.dropdown1.set("Seleccione opciones")
        self.dropdown1.bind("<Button-1>", lambda e: self.show_multiselect("Géneros", ["Action", "Adventure", "Animation", "Children´s", "Comedy", "Crime", "Documentary", "Drama", "Fantasy", "Film-Noir", "Horror", "Musical", "Mystery", "Romance", "Sci-Fi", "Thriller", "War", "Western"], self.dropdown1, "generos"))
        self.dropdown1.pack(pady=5)

        # Desplegable de epoca
        self.label2 = tk.Label(self, text="Época:", font=("Corleone Due", 30), bg="lightblue")
        ...
        # Marco para la cantidad de recomendaciones
        self.n_movies_button_frame = tk.Frame(self, bg="lightblue")
        self.n_movies_button_frame.pack(pady=20)
        ...
        self.n_movies_label = tk.Label(self.n_movies_button_frame, text="ID:", font=("Milky Vintage", 20), bg="lightblue")
        self.n_movies_label.grid(row=0, column=0, padx=5)
        self.n_movies_entry = tk.Entry(self.n_movies_button_frame, textvariable=self.user_id, font=("Milky Vintage", 20), width=10, justify="center")
        self.n_movies_entry.grid(row=0, column=1, padx=5)
        self.n_movies_entry.config(validate="key", validatecommand=(self.register(self.validate_id), "%P"))
    ...
    def recommend(self):
        ...
        resultado = recomendacion.recomendar(user= int(user_id_text), generos= app.get_user_selection()["generos"], decadas = app.get_user_selection()["epoca"], tags = app.get_user_text_tags(), top_n = 5)
        resultado_legible = "\n".join([str(r) for r in resultado])
        messagebox.showinfo(
            "Recomendación",
            f"ID de usuario: {user_id_text}\n\nSelección del usuario:\n{selected_categories}\n\nRecomendaciones:\n\n{resultado_legible}"
        )

if __name__ == "__main__":
    recomendacion = RecommendationSystem()
    recomendacion.cargar_modelo()
    app = Interfaz()
    app.mainloop()  
